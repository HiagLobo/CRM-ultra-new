...
describe("código enviado", () => {
  it("cadastro novo: sem aviso de 'já tem cadastro' e sem atualizacao no verify", () => {
    ...
    expect(e).toMatchObject({ passo: "codigo", codigoDev: "123456", emailCriado: DADOS.email });
...
describe("liberado", () => {
  it("guarda os campos não atualizados para o aviso do passo final", () => {
    expect(fluxo(estadoInicial(), { tipo: "verificado", naoAtualizados: ["telefone"] })).toMatchObject({
      passo: "ok",
      naoAtualizados: ["telefone"],
    });
    expect(fluxo(estadoInicial(), { tipo: "verificado" }).naoAtualizados).toEqual([]);
  });
});
